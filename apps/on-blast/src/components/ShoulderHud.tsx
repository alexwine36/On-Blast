 ...
 */
export function ShoulderHud({ posture }: ShoulderHudProps) {
  const rows = [
    {
      label: "Arm spread",
  ...
  return (
    <section className="gesture">
      <header className="gesture__head">
        <span className="gesture__title">Shoulders</span>
        <span className={`gesture__state ${posture.armsOut ? "is-ok" : ""}`}>{posture.reason}</span>
      </header>
      ...
      <p className="gesture__hint">
        Gap shrinks as shoulders rise toward the ears. Relax then shrug, and read the two
        extremes here to set the range.
      </p>
    </section>
  );
}
